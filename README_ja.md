...
[OuterTune/OuterTune](https://github.com/OuterTune/OuterTune) `v0.10.2` をベースに以下の変更を加えています：

- YouTube Music の複数バグを修正（アルバムトラック未表示、プレイリストクラッシュ、検索結果パースエラー）
- 歌詞取得の精度向上と表示遅延の改善（LrcLib + キャプショントラック使用）
